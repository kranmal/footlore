...
/**
 * Look a stop up. The fixture catalogue is the default so the tests and the
 * prototype keep working untouched; the live app passes `plan.catalog`, which
 * is the whole reason the solver can run on real places at all.
 */
export const place = (id, catalog = CATALOG) => {
  const p = catalog[id];
...
export function schedule(plan) {
  ...
  for (const s of plan.stops) {
    const p = place(s.id, plan.catalog);
  ...
  const dwelling = stops.filter((s) => !place(s.id, plan.catalog).window).reduce((n, s) => n + s.dwell, 0);
  const mealTime = stops.filter((s) => place(s.id, plan.catalog).window).reduce((n, s) => n + s.dwell, 0);
