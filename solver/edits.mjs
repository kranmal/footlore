...
export function diff(before, after) {
  ...
  const phrase = [];
  if (added.length) phrase.push(`added ${added.map((id) => place(id).name).join(', ')}`);
  if (removed.length) phrase.push(`dropped ${removed.map((id) => place(id).name).join(', ')}`);
...
export function applyEdit(plan, edit) {
  switch (edit.type) {
    ...
    case 'unpin': {
      ...
      return { plan: next, diff: diff(plan, next), note: edit.type === 'pin' ? `${place(edit.id).name} will stay put` : null };
    }
    ...
    case 'drop': {
      ...
      const alt = solve({ ...next, budget: plan.budget }, {
        ...
        mealId: next.stops.find((s) => place(s.id).window)?.id ?? null,
        exclude: [edit.id],
      });
      ...
      return {
        ...
        offer: {
          ...
          options: [
            ...
            suggestion
              ? { label: `Add ${place(suggestion).name} instead`, action: { type: 'add', id: suggestion } }
    ...
    case 'setBudget': {
      const res = solve({ ...plan, budget: edit.minutes }, {
        ...
        mealId: plan.stops.find((s) => place(s.id).window)?.id ?? null,
      });
